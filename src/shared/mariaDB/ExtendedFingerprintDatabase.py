...
class ExtendedFingerprintDatabase(FingerprintDatabase):
    ...
    def export_database_to_dataframe(self, company: str, group: str) -> pd.DataFrame:
        """
        Export the entire database associated with the given company and group to a DataFrame.
    ...
    @FingerprintDatabase.requires_connection
    @FingerprintDatabase.log_execution
    def import_dataframe_to_database(self, company: str, group: str, dataframe: pd.DataFrame) -> None:
        """
        Import data from a DataFrame to the database associated with the given company and group.
        ...
        existing_fingerprints, _ = self.read_fingerprints(company, group)  # Read existing fingerprints from the database
        
        with self.session_scope() as session:
            for fingerprint in fingerprints:
                if fingerprint not in existing_fingerprints:  # Check if the fingerprint already exists
                    self.save_fingerprints(fingerprint, company, group)

    @FingerprintDatabase.requires_connection
    @FingerprintDatabase.log_execution
    def import_dataframe_to_whole_database(self, dataframe: pd.DataFrame) -> None:
        """
        ...
        existing_fingerprints, _ = self.read_fingerprints_for_all()  # Read existing fingerprints from the database
        
        with self.session_scope() as session:
            for fingerprint, company, group in zip(fingerprints, companies, groups):
                if fingerprint not in existing_fingerprints:  # Check if the fingerprint already exists
                    self.save_fingerprints(fingerprint, company, group)