...
class FingerprintDatabase:
    def __init__(self, host: str, user: str, password: str, database: str, used_sqlite: bool = False):
        self.used_sqlite = used_sqlite
        if used_sqlite:
            self._db_url = f"sqlite:///{database}"
        else:
            self._db_url = f"mysql+mysqlconnector://{user}:{password}@{host}/{database}"
    ...
    def log_execution(method):
        def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                logger.info(f"{current_time} - Method: {method_name} | Args: {method_args} | Kwargs: {method_kwargs} | Execution Time: {execution_time:.4f} seconds")
                return result
            except Exception as e:
                end_time = time.time()
                execution_time = end_time - start_time
                current_time = time.strftime('%Y-%m-%d %H:%M:%S')
                error_message = str(e)
                logger.error(f"{current_time} - Method: {method_name} | Args: {method_args} | Kwargs: {method_kwargs} | Execution Time: {execution_time:.4f} seconds | Error: {error_message}")
                raise e
        return wrapper
    ...
    def save_fingerprints(
        ...
        :param fingerprints: Fingerprint(s) to save. Can be a single fingerprint string or a list of fingerprint strings.
        ...
        with self.session_scope() as session:
            ...
            for fingerprint in fingerprints:
                ...
                try:
                    ...
                    session.rollback()
                    # Handle the case where the entry already exists (e.g., log an error, raise an exception, etc.)
    ...
    def read_fingerprints(
        self, company: Optional[str] = None, group: Optional[str] = None, ids_tabla: Union[int, List[int]] = None
    ) -> Tuple[List[str], List[int]]:
        """
        Read fingerprints from the database for the specified company, group, and/or table IDs.
        ...
        :param ids_tabla: ID(s) of the fingerprint entries to retrieve (optional).
                          Can be a single integer ID or a list of integer IDs.
        :type ids_tabla: Union[int, List[int]] or None

        :return: Tuple containing lists of fingerprints and their corresponding table IDs.
        ...
        with self.session_scope() as session:
            ...
            if ids_tabla is not None:
                if isinstance(ids_tabla, int):
                    ...
                    raise ValueError("ids_tabla should be an integer or a list of integers.")
    ...
    def read_fingerprints_to_dataframe(
        ...
        :rtype: pd.DataFrame
        """
        fingerprints, table_ids = self.read_fingerprints(company, group, ids_tabla)
    ...
    def update_fingerprint(
        ...
        """
        Update fingerprints in the database with the given IDs and/or fingerprints.
        ...
        :param fingerprints: New fingerprint(s) to update with.
                             Can be a single fingerprint string or a list of fingerprint strings.
        :type fingerprints: Union[str, List[str]]
        ...
        if isinstance(ids_tabla, int):
            ...
            if len(ids_tabla) != len(fingerprints):
                raise ValueError("The lists of IDs and fingerprints must have the same size.")
        else:
            raise ValueError("Invalid input. Both ids_tabla and fingerprints should be either integers or lists.")
        ...
        with self.session_scope() as session:
            ...
            for fingerprint_id, new_fingerprint in zip(ids_tabla, fingerprints):
    ...
    def delete_fingerprints(
        ...
        """
        Delete fingerprints from the database for the specified company, group, and/or table IDs.
        ...
        :param ids_tabla: ID(s) of the fingerprint entries to delete (optional).
                          Can be a single integer ID or a list of integer IDs.
        :type ids_tabla: Union[int, List[int]] or None
        """
        with self.session_scope() as session:
    ...
    @requires_connection
    @log_execution
    def count_fingerprints(self, company: Optional[str] = None, group: Optional[str] = None) -> int:
        """
        Count the number of fingerprints in the database for the specified company and/or group.
    ...
    def generate_histogram(self) -> Tuple[Image.Image, List[Tuple[str, int]]]:
        """
        Generate a histogram of various categories in the database and return the image buffer and data matrix.

        :return: Tuple containing the image buffer and data matrix.
        :rtype: Tuple[Image.Image, List[Tuple[str, int]]]
        """
        with self.session_scope() as session:
            query = session.query(FingerprintEntry.company, FingerprintEntry.group, func.count(FingerprintEntry.id))
            query = query.group_by(FingerprintEntry.company, FingerprintEntry.group)
            results = query.all()
            ...
            labels = ['Company', 'Group', 'Company-Group', 'No Company-Group', 'Company-No Group', 'No Company-No Group']
            sizes = [
    ...
    def delete_database_sqlite(self):
        ...
        This method will close the existing database connection, if any, and then delete the database file.

        Note: Use this method with caution as it will permanently delete the database file and its contents.
        """
        if self.used_sqlite:
            raise RuntimeError("This method can only be used with SQLite databases.")

        if self._is_connected:
            self.engine.dispose()  # Close the existing connection if it's open
        
        # Delete the database file if it exists
        if os.path.exists(self._db_url[10:]):  # Removing 'sqlite:///' from the beginning
